...
BossSprites BossLoadSprites(const char *jsonPath) {
    BossSprites sprites = {0};
    char *data = NULL;
    cJSON *root = LoadJSONFile(jsonPath, &data);
    if (!root) return sprites;

    cJSON *walk = cJSON_GetObjectItem(root, "boss_walk");
    cJSON *attack1 = cJSON_GetObjectItem(root, "boss_attack1");
    cJSON *attack2 = cJSON_GetObjectItem(root, "boss_attack2");
    cJSON *attack3 = cJSON_GetObjectItem(root, "boss_attack3");
    cJSON *attack4 = cJSON_GetObjectItem(root, "boss_attack4");
    ...
    if (attack1) {
        sprites.attack_right1 = LoadAnimationFromArray(cJSON_GetObjectItem(attack1, "right"));
        sprites.attack_left1  = FlipAnimationHorizontally(sprites.attack_right1);
    }
    if (attack2) {
        sprites.attack_right2 = LoadAnimationFromArray(cJSON_GetObjectItem(attack2, "right"));
        sprites.attack_left2  = FlipAnimationHorizontally(sprites.attack_right2);
    }
    if (attack3) {
        sprites.attack_right3 = LoadAnimationFromArray(cJSON_GetObjectItem(attack3, "right"));
        sprites.attack_left3  = FlipAnimationHorizontally(sprites.attack_right3);
    }
    if (attack4) {
        sprites.attack_right4 = LoadAnimationFromArray(cJSON_GetObjectItem(attack4, "right"));
        sprites.attack_left4  = FlipAnimationHorizontally(sprites.attack_right4);
    }
...
void BossUnloadSprites(BossSprites s) {
    UnloadAnimation(s.walk_right);
    UnloadAnimation(s.walk_left);
    UnloadAnimation(s.attack_right1);
    UnloadAnimation(s.attack_left1);
    UnloadAnimation(s.attack_right2);
    UnloadAnimation(s.attack_left2);
    UnloadAnimation(s.attack_right3);
    UnloadAnimation(s.attack_left3);
    UnloadAnimation(s.attack_right4);
    UnloadAnimation(s.attack_left4);
}

BossSprites LoadBossSprites(const char *jsonPath) {
    return BossLoadSprites(jsonPath);
}
